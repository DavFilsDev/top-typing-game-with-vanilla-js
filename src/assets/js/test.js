document.addEventListener('DOMContentLoaded', function() {
    ...
    // Variables pour le chasseur (crocodile)
    let hunterPosition = 0;
    let hunterSpeed = 1; // sera ajusté selon le mode
    let hunterInterval = null;
    const HUNTER_BASE_SPEED = 0.5; // vitesse de base en px/frame


    // Fonction pour déplacer le chasseur
    const moveHunter = () => {
        if (!testActive || isPaused) return;
        
        // Calculer la position cible (derrière le curseur de l'utilisateur)
        const targetPosition = Math.max(0, currentPosition - 10);
        
        // Déplacer progressivement le chasseur vers la cible
        if (hunterPosition < targetPosition) {
            hunterPosition += hunterSpeed;
            updateHunterPosition();
        }
        
        // Vérifier si le chasseur a attrapé l'utilisateur
        if (hunterPosition >= currentPosition) {
            userCaught();
        }
    };

    // Mettre à jour la position visuelle du chasseur
    const updateHunterPosition = () => {
        if (currentPosition >= currentText.length) return;
        
        const charElement = textDisplay.querySelectorAll('.char')[Math.floor(hunterPosition)];
        ...
        hunterImg.style.left = `${rect.left - containerRect.left - 30}px`;
        hunterImg.style.top = `${rect.top - containerRect.top - 10}px`;
        
        // Faire disparaître les lettres derrière le chasseur
    ...
    // Fonction quand l'utilisateur est attrapé
    const userCaught = () => {
        clearInterval(hunterInterval);
        endTest(true);
        
        // Modifier le message de résultats
        document.querySelector('#results h2').textContent = 
            currentLang === 'fr' ? 'Le crocodile a gagné !' : 'Crocodile wins!';
        document.querySelector('#results').innerHTML += `
            <div class="mt-4">
                <img src="/public/img/crocodile-win.png" alt="Winner" class="h-24 mx-auto">
                <p class="mt-2">${currentLang === 'fr' 
    ...
    // Modifiez la fonction endTest pour accepter un paramètre "caught"
    const endTest = (caught = false) => {
        clearInterval(timer);
        clearInterval(hunterInterval);
        timer = null;
        testActive = false;
        ...
        if (!caught) {
            // Calculs normaux de WPM si l'utilisateur a gagné
            ...
            // Message de victoire
            document.querySelector('#results h2').textContent = 
                currentLang === 'fr' ? 'Vous avez gagné !' : 'You won!';
            document.querySelector('#results').innerHTML += `
                <div class="mt-4">
                    <img src="/public/img/crocodile-lose.png" alt="Loser" class="h-24 mx-auto">
                    <p class="mt-2">${currentLang === 'fr' 
                        ? 'Vous avez échappé au crocodile !' 
                        : 'You escaped the crocodile!'}</p>
                </div>
            `;
        }
    ...
    const generateText = () => {
        ...
        // Réinitialiser l'opacité de tous les caractères
    ...
    const initTest = () => {
        ...
        // Définir la vitesse selon le mode
        const mode = document.getElementById('mode').value;
        hunterSpeed = HUNTER_BASE_SPEED * 
            (mode === 'easy' ? 1 : mode === 'medium' ? 1.5 : 2);
        
        // Démarrer le mouvement du chasseur
        if (hunterInterval) clearInterval(hunterInterval);
        hunterInterval = setInterval(moveHunter, 50);
    };
